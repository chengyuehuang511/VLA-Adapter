    ...
    def adam(self, group,
        ...
        for i, (param, name) in enumerate(zip(group['params'],group['name'])):
            ...
            if param.grad is None: 
                continue
            grad = param.grad
            exp_avg = exp_avgs[i_with_grad]
            exp_avg_sq = exp_avg_sqs[i_with_grad]
            step = state_steps[i_with_grad]
            if amsgrad:
                max_exp_avg_sq = max_exp_avg_sqs[i_with_grad]
            ...
            # exp_avg shape: torch.Size([768, 3, 16, 16])
            # grad shape: torch.Size([768])
